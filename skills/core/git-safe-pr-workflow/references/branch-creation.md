...
- **Non-empty output:** show the commit list and ask the user:
...
If checkout fails because uncommitted changes would be overwritten, stash and retry. The stash stack is shared across worktrees, so use a unique tag and restore by SHA rather than a bare `git stash pop`:

```bash
git stash push -u -m "safe-pr-workflow: pre-branch <branch-name>"
git checkout -b <branch-name> "$BASE_REF"
git stash pop
```
