...
Loaded by `SKILL.md` once the interview is complete. Fill it in from the captured answers and write to `STRATEGY.md`.

## Rules for filling in

- Use the user's own language where possible. Do not paraphrase into generic PM-speak.
- Each section stays compact. The whole doc should read in under five minutes.
...
- Set `last_updated` in the frontmatter to today's ISO date (YYYY-MM-DD). Do not duplicate the date in prose.
