...
Loaded by `SKILL.md` at the start of the first-run interview and revisited per-section on an update run. Every section below maps one-to-one to a section in `strategy-template.md`.
...
1. **Ask, don't prescribe.** Do not offer menu options for the open answers (problem, approach, persona). Use free-form responses.
2. **Push back once, maybe twice.** If the first answer is weak, name the specific issue and ask a sharper question. If the second answer is still weak, capture what the user gave and note in the draft that the section is worth revisiting. Do not let the interview spiral.
...
Strong answers name a specific situation the target user is in, identify what makes it hard *right now* (a crux, a constraint, something that isn't easy to route around), and are falsifiable — you could imagine the problem being absent and know the difference.
...
- **Product description as approach** ("we use AI to draft replies") → "That's what the product does, but what's the *choice* inside it? Every competitor will say the same thing. Name what you're doing that the obvious alternative isn't — a grounding choice, a trust-building commitment, a workflow bet? What are you betting on that they're not?"
...
Strong answers name one primary persona (secondary personas allowed), identify them by role or situation rather than demographic, and state a concrete job as a verb phrase.
...
**Capture:** A list of 3–5, each with a one-line definition. Note where each is measured (analytics, DB, qualitative) if known. If measurement is undefined, ask: "Where does this metric live today? If nowhere, is this something you can start measuring?"
...
Strong answers stay at 2–4, connect clearly back to the approach, and are broad enough that multiple features live inside each one.

**Anti-patterns and pushback:**

- **Feature list in disguise** ("track 1: Slack integration; track 2: mobile app; track 3: dark mode") → "Those are features. What's the *investment area* each lives inside? 'Integrations' might be one track, with Slack, Teams, and Discord as candidates inside it."
...
**Capture:** 2–4 tracks. For each: a name, a one-line purpose, and a short note on why it serves the approach.
...
Only capture externally visible, real milestones. Default is to skip; do not push the user to invent milestones. If they name some, capture them verbatim with dates.
...
Once sections 1–5 are captured (plus any optional sections the user engaged with), read `strategy-template.md`, fill it in, present the full draft in chat, offer one edit round, then write `STRATEGY.md`.
