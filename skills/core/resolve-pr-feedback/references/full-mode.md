...
2. Push to remote, then **verify the push landed** (the verified-push boundary in
   git:safe-pr-workflow): after `git push`, freshly query the exact destination ref and require the
   returned remote object ID to equal local `HEAD`. A zero exit status or a cached remote-tracking
   branch does not prove delivery.
```bash
git push
git ls-remote --exit-code origin "refs/heads/$(git branch --show-current)"   # confirm the ref, compare its OID to local HEAD
```
If the ref query fails, the ref is missing, or the OIDs differ, treat it as `BLOCKED`/`FAIL` and report
— do not proceed to reply/resolve as if the fixes shipped. Never `--no-verify` or force-push.
