...
4. Add or update tests for changed behavior. Use `tdd` at the pre-agreed seams, where a failing test can lead the change. If a useful test is not practical, say why and verify manually.
5. Run the most relevant checks first — typecheck and single test files regularly, the full suite once at the end. Widen checks when shared or user-facing behavior changed. Fix relevant failures without expanding the task.
