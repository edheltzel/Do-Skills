...
description: >-
  Repo-wide structural-maintainability review — code-judo restructurings, 1k-line file
  guard, anti-spaghetti branching, canonical-layer enforcement, anti-magic abstractions,
  explicit type/boundary contracts. Use when asked for a structural review, an architecture
  health check on a branch, "is this getting messy", or before merging a large change.
  NOT FOR correctness bug hunting (use adversarial-review) or small local cleanups
  (use simplify) — this is the ambitious, whole-repo structural lens.
---
...
1. **Do not let a PR push a file from under 1k lines to over 1k lines without a very strong reason.**
   ...
   - This guard measures **file** size, not interface size. A deep module (see the codebase-design skill) can legitimately hold a large implementation behind a small interface. When a file crosses the budget, the remedy is to **extract internal seams** — split the implementation into private files behind the *same* interface — not to widen the interface to shed lines. Decomposing for file size keeps the module deep; widening the interface makes it shallow.
...
On top of this repo's documented standards, carry this fixed set of Fowler code smells (_Refactoring_, ch.3) as a decomposition-trigger checklist against the diff. Two rules bind the whole table:

- **Repo standards override.** A documented repo standard always wins; where it endorses something the baseline would flag, suppress the smell.
- **Always a judgement call, and skip what tooling enforces.** Each smell is a labelled heuristic ("possible Feature Envy"), never a hard violation. If a linter or formatter already enforces it, don't re-flag it here.
...
Several smells (Duplicated Code, Speculative Generality, Message Chains, Middle Man) restate rules 0–7 from a different angle; when they overlap, report once under the stronger frame.
