...
```bash
bun ~/.claude/skills/do-art/Tools/PickExpression.ts --topic "<TOPIC>"
...
```bash
bun ~/.claude/skills/do-art/Tools/Generate.ts --workflow=StyleMatchedThumbnail --model nano-banana-pro \
...
```bash
bun ~/.claude/skills/do-art/Tools/ThumbnailText.ts \
...
```bash
bun ~/.claude/skills/do-art/Tools/ThumbnailText.ts --mode interview \
