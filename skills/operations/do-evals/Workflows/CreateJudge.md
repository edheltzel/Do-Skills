...
```yaml
judge:
  name: do-<Descriptive Name> Judge
  ...
  ai_based:
    - scorer: "custom-judge"
      ...
      params:
        ...
        judge_response: "<explicit response from the active judge adapter>"
```
...
```yaml
judge:
  name: do-Factual Accuracy Judge
  focus: accuracy
  scale:
    type: 1-5
  criteria:
    - name: do-Factual Correctness
      description: All claims match source material
      weight: 0.5
    - name: do-Completeness
      description: Covers all key points from source
      weight: 0.3
    - name: do-No Hallucinations
...
```yaml
judge:
  name: do-Voice Authenticity Judge
  focus: style
  scale:
    type: 1-5
  criteria:
