...
> **Atlas note.** The definitions below describe the general skill-system mechanic, in which user-invocation is realised by removing a skill's **description**. This atlas collection keeps `name` and `description` on _every_ skill (its frontmatter contract), so here a "user-invoked" skill is one whose description is written _lean_ — a human-facing summary without agent triggers — rather than one with the description stripped. The axis, the two loads, and every lever are identical; only the lever's implementation differs.

### Model-Invoked

A skill that keeps its **description** field, so the agent can see it and fire it autonomously — and the human can still type its name, so model-invocation always _includes_ user reach. There is no model-only state: a description only ever _adds_ agent discovery, never removes the human's. Pays a permanent **context load** on every turn in exchange for that discoverability. Reachable by other skills, because the description that makes it agent-discoverable makes it invocable. A model-invoked skill whose content is all **reference** is also one home for shared reference: another skill can invoke it, so reference needed by several skills lives in one place. Pick model-invocation only when the agent must reach the skill on its own; if it never fires except by hand, drop the triggers and pay no attention cost.

_Avoid_: ability, tool, capability

### User-Invoked

A skill with its **description** stripped (or, in atlas, written lean and trigger-free) — invisible to the agent and reachable only by the human typing its name (user-_only_, where **model-invoked** is user-_and-agent_). Trades agent-discoverability for near-zero **context load**. Because nothing agent-facing describes it, nothing but the human can reach it: no other skill can fire it.

_Avoid_: procedure, workflow, command

### Description

The skill's machine-readable trigger, and the one **context pointer** a **model-invoked** skill is forced to keep loaded at all times. Its trigger phrasing _is_ the invocation axis: write it trigger-rich and the skill is model-invoked (and reachable by other skills); write it lean and human-facing and the skill is **user-invoked**, reachable in practice only by the human. The source of a model-invoked skill's **context load**.
...
The cost a **model-invoked** skill imposes on the agent's context window — its **description**, always loaded, spending both tokens and attention. What **user-invoked** skills escape by having no agent-facing triggers, and the brake on splitting into more model-invoked skills.
...
A **user-invoked** skill whose job is to point at your other user-invoked skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It can only hint, never fire them: user-invoked skills have no agent-facing **description**, so nothing but the human can reach them. The cure for **cognitive load** when user-invoked skills multiply.
...
**Reference** that lives outside the skill system — a plain file, no **description**, no **steps**, not invocable — that any skill can point at. The home for shared reference that needn't fire on its own, and the only shared home two **user-invoked** skills can use, since neither has an agent-facing description and so neither can fire the other.
