---
name: loop-me
description: Grill me about the specs for the workflows I want to build, within a stateful workspace — the loop/workflow/trigger/checkpoint/push-right/brief vocabulary. USE WHEN you type its name to design or spec a workflow; it runs a grilling session whose only output is workflow specs. Name a workflow to design, or nothing to go find one. NOT FOR general plan or design grilling (use grilling).
---

Run a stateful grilling session whose only output is **workflow** specs. Use the grilling discipline — relentless, one question at a time, a recommended answer attached to each — aimed at the vocabulary and goal below. Create, edit, and delete specs as the grilling resolves things. If the user named a workflow, design that one; if not, use the lens to go find one worth specifying.
