...
```bash
bun ~/.agents/skills/do-art/Tools/PickExpression.ts --topic "<TOPIC>"
...
```bash
bun ~/.agents/skills/do-art/Tools/Generate.ts --workflow=StyleMatchedThumbnail --model nano-banana-pro \
...
```bash
bun ~/.agents/skills/do-art/Tools/ThumbnailText.ts \
...
```bash
bun ~/.agents/skills/do-art/Tools/ThumbnailText.ts --mode interview \
  --kicker "A CONVERSATION WITH" --title "GRANT LEE" --subtitle "ON BUILDING GAMMA" \
  --face host.png --face2 guest.png --name1 "Ed" --name2 "Grant Lee" \
...
- **Logo:** the real "TI:" node-mark, extracted to `~/.agents/Assets/brand/ti-logo-white.png`, composited top-right inside the border. `--no-logo` to disable.
