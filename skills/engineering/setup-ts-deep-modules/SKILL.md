...
description: >-
  ...
  implementation hidden in subfolders, reachable only through its entry-point files — and
  prove the rules bite. USE WHEN enforcing package boundaries, hiding package internals,
  stopping deep imports across packages, or making a TS monorepo's packages deep modules.
  NOT FOR the deep-module vocabulary itself (use codebase-design) or Husky/Prettier
  commit-time checks (use setup-pre-commit). User-invoked.
---
...
For the vocabulary (deep module, interface, seam, depth), use the codebase-design skill — use its language throughout.
