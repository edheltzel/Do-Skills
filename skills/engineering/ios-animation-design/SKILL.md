...
description: Design and spec iOS animations before code is written — structured motion specs covering transitions, micro-interactions, gesture-driven motion, and loading states, each with interruption, Reduce Motion, and haptics decided up front. USE WHEN planning, designing, or spec'ing app motion, choosing between animation approaches, or working out screen transitions, navigation animations, interactive gestures, onboarding flows, or loading states. NOT FOR writing the animation code (use ios-animation-implementation) or non-iOS motion work.
---
...
Once an approach is chosen, hand the compiled spec to ios-animation-implementation to write the code. Both skills sit under the ios-development umbrella; route end-of-session Swift polish to cleanup-swift.
