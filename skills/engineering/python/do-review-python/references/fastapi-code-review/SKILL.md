...
Run **once per FastAPI-related finding**, after you can anchor **`file:line`** for the handler (see [review-verification-protocol](../review-verification-protocol.md)) and **before** the finding text ships. If a step’s pass condition is not met, **do not** assert the finding as written—gather evidence, withdraw, downgrade severity, or rephrase as a question.
...
2. Load and follow [review-verification-protocol](../review-verification-protocol.md) (Pre-Report checklist and **Verification by Issue Type**) before reporting any issue.
