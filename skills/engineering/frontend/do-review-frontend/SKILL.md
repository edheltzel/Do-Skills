...
Load the [review-verification-protocol](references/review-verification-protocol.md) skill before any substantive judgment on code.
...
- Load [review-remix-v2](references/review-remix-v2/SKILL.md) — umbrella orchestrator that loads all six Remix v2 area review skills (routing, data-flow, forms, error-boundaries, perf/SSR, meta+sessions) with its own detection telemetry. Run it for the Remix-specific surface and consolidate its findings into this review's output.
- Do NOT also load [react-router-code-review](references/react-router-code-review/SKILL.md) — Remix v2 sits on React Router v6 but routes through Remix's route module conventions, which [remix-v2-routing-review](references/remix-v2-routing-review/SKILL.md) (loaded via the umbrella) covers in that context.
- Still load [shadcn-code-review](references/shadcn-code-review/SKILL.md) and the conditional skills below — they're orthogonal to the Remix layer and apply to component/styling/state code regardless of router.

**Non-Remix branch (default):**

- [react-router-code-review](references/react-router-code-review/SKILL.md)
- [shadcn-code-review](references/shadcn-code-review/SKILL.md)

**Conditionally load based on detection (both branches):**

| Condition | Skill |
|-----------|-------|
| @xyflow/react detected | [react-flow-code-review](references/react-flow-code-review/SKILL.md) |
| Zustand detected | [zustand-state](references/zustand-state/SKILL.md) |
| Tailwind v4 detected | [tailwind-v4](references/tailwind-v4/SKILL.md) |
| Test files changed | [vitest-testing](references/vitest-testing/SKILL.md) |
...
2. **Protocol + branch skills loaded** — **Pass when:** [review-verification-protocol](references/review-verification-protocol.md) and [shadcn-code-review](references/shadcn-code-review/SKILL.md) are loaded, **and** either (a) Step 2 found Remix v2 and [review-remix-v2](references/review-remix-v2/SKILL.md) is loaded (without [react-router-code-review](references/react-router-code-review/SKILL.md)), or (b) Step 2 found no Remix v2 and [react-router-code-review](references/react-router-code-review/SKILL.md) is loaded — before first severity judgment.
