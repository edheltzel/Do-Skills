...
3. **Protocol gate:** the [review-verification-protocol](references/review-verification-protocol.md) skill is loaded before Step 7. **Pass:** Every Critical/Major finding satisfies Step 8 (and the protocol); if there are zero findings, say "Protocol applied; no issues" in the Review Summary.
...
- If atomics (`std::sync::atomic`, `compare_exchange`, `fetch_*`), `UnsafeCell`, `unsafe impl Send/Sync`, or `crossbeam` / `arc-swap` / `parking_lot` are present in the diff, load the [rust-code-review](references/rust-code-review/SKILL.md) skill and consult its `references/concurrency-primitives.md`, `references/memory-ordering.md`, and `references/lock-free-patterns.md`.
...
- If hand-rolled atomics / lock-free types appear with no `loom` dependency or no `cargo +nightly miri test` in CI, load the [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) skill and consult its `references/concurrency-testing.md`.

**Interface design / API surface detection:**
- If the diff introduces or changes `pub trait`, `pub fn`, `pub struct`, derive impls on public types, `impl Drop` on owning types, or re-exports of foreign types, load the [rust-code-review](references/rust-code-review/SKILL.md) skill and consult its `references/interface-design.md` for object-safety, ergonomic-impl, fallible-destructor, and hidden-contract review checks.
...
- If `benches/` directory or `criterion` dependency is present, load the [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) skill and consult its `references/advanced-testing.md` for criterion baseline, `black_box`, and `iter_batched` review checks.
- If proc-macro crate (`proc-macro = true`) or `trybuild` in `[dev-dependencies]`, consult the [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) skill's `references/advanced-testing.md` for trybuild `.stderr` stability checks plus the [macros-code-review](references/macros-code-review/SKILL.md) skill's `references/procedural-macros.md` for span hygiene and `syn` feature audits.

## Step 5: Load Verification Protocol

Load the [review-verification-protocol](references/review-verification-protocol.md) skill and keep its checklist in mind throughout the review.

## Step 6: Load Skills

Load each applicable skill below (e.g. load the [rust-code-review](references/rust-code-review/SKILL.md) skill) by reading its `SKILL.md` and applying it.

**Always load:**
- [rust-code-review](references/rust-code-review/SKILL.md)

**Conditionally load based on detection:**

| Condition | Skill |
|-----------|-------|
| Tokio detected | [tokio-async-code-review](references/tokio-async-code-review/SKILL.md) |
| Axum detected | [axum-code-review](references/axum-code-review/SKILL.md) |
| sqlx detected | [sqlx-code-review](references/sqlx-code-review/SKILL.md) |
| Serde detected | [serde-code-review](references/serde-code-review/SKILL.md) |
| Test files changed | [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) |
| Macro definitions in diff | [macros-code-review](references/macros-code-review/SKILL.md) |
| FFI code detected (extern, repr(C), bindgen) | [ffi-code-review](references/ffi-code-review/SKILL.md) |
| Atomics, `UnsafeCell`, `unsafe impl Send/Sync`, `compare_exchange`, `crossbeam`, `arc-swap`, `parking_lot` | [rust-code-review](references/rust-code-review/SKILL.md) (load `references/concurrency-primitives.md`, `references/memory-ordering.md`, `references/lock-free-patterns.md`) |
| Concurrency design changes (worker pools, channels, threads-vs-async restructuring) | [rust-code-review](references/rust-code-review/SKILL.md) (load `references/concurrency-models.md`) |
| Public trait / `pub fn` / `pub struct` / `impl Drop` / re-export changes | [rust-code-review](references/rust-code-review/SKILL.md) (load `references/interface-design.md`) |
| Graph/tree code, `slotmap`, `petgraph`, `mem::replace` drop guards, extension traits, `prelude` module changes | [rust-code-review](references/rust-code-review/SKILL.md) (load `references/patterns-in-the-wild.md`) |
| `criterion` benchmarks, `benches/` directory | [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) (load `references/advanced-testing.md` — criterion baseline + `black_box` + `iter_batched` checks) |
| Proc-macro crate (`proc-macro = true`) or `trybuild` in dev-deps | [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) + [macros-code-review](references/macros-code-review/SKILL.md) (load `references/advanced-testing.md` trybuild + `references/procedural-macros.md` span hygiene) |
| `loom`, `miri`, hand-rolled lock-free code under test | [rust-testing-code-review](references/rust-testing-code-review/SKILL.md) (load `references/concurrency-testing.md`) |
