...
description: Write iOS animation code with Apple's first-party frameworks — SwiftUI animations, Core Animation, and UIKit — preferring native APIs over third-party libraries. USE WHEN implementing iOS animations, building transitions, creating gesture-driven interactions, or turning an animation spec into working Swift (KeyframeAnimator, PhaseAnimator, custom Transition protocol, zoom navigation transitions, matchedGeometryEffect, symbol effects, mesh gradients, SwiftUI-UIKit bridging across iOS 18–26). NOT FOR planning or spec'ing the motion first (use ios-animation-design) or non-iOS platforms.
---
...
Work from a spec produced by ios-animation-design when one exists — it already resolves trigger, interruption, Reduce Motion, and haptics. This skill lives under the ios-development umbrella; after the animation code is in place, route end-of-session Swift polish to cleanup-swift.
